...
export async function getDataActivitiesTotal(token:string|null){
    try{
        if(!token) return;
        const url = `${process.env.NEXT_PUBLIC_API_URL}/activities/total`;
...
export default getDataActivitiesTotal;