...
export async function getDataActivitiesSuccess(token:string|null){
    try{
        if(!token) return;
        const url = `${process.env.NEXT_PUBLIC_API_URL}/activities/total-success`;
...
export default getDataActivitiesSuccess;