...
export async function getDataCountPremium(token:string|null){
    try{
        if(!token) return;
        const url = `${process.env.NEXT_PUBLIC_API_URL}/users/total-premium`;
...
export default getDataCountPremium;