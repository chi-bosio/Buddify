...
export async function getDataActivitiesPending(token:string|null){
    try{
        if(!token) return;
        const url = `${process.env.NEXT_PUBLIC_API_URL}/activities/total-pending`;
...
export default getDataActivitiesPending;