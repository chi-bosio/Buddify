...
export async function getDataUsersBannedTotal(token:string|null){
    try{
        if(!token) return;
        const url = `${process.env.NEXT_PUBLIC_API_URL}/users/total-banned`;
...
export default getDataUsersBannedTotal;