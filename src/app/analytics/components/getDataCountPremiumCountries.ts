...
export async function getDataCountPremiumCountries(token:string|null){
    try{
        if(!token) return;
        const url = `${process.env.NEXT_PUBLIC_API_URL}/users/premium-countries`;
...
export default getDataCountPremiumCountries;