    ...
    <nav className="relative bg-customPalette-black flex items-center justify-between px-3">
      ...
      {admin && isLoggedIn && (
        <button
          className={`${
            pathname === "/analytics"
              ? `bg-customPalette-orange 
            hover:bg-customPalette-orangebright
            text-customPalette-white rounded px-2`
              : `bg-customPalette-white 
            hover:bg-customPalette-whitelight 
            text-customPalette-black rounded `
          } 
            duration-300 ease-in-out transition-all px-3 py-1 text-xs xl:text-lg
              `}
          onClick={() => {
            // eslint-disable-next-line @typescript-eslint/no-unused-expressions
            pathname === "/analytics"
              ? handlerRedirectDashboardAdmin("/")
              : handlerRedirectDashboardAdmin("analytics");
          }}
        >
          {pathname === "/analytics" ? "Ver como usuario" : "Ver como Admin"}
        </button>
      )}
      {!isAnalyticsPage && (
        <>
          <div className="flex lg:flex-row-reverse z-10">
            {isLoggedIn && (
              <div className="z-50 lg:z-20 relative flex items-center justify-center">
                <Crown
                  isPremium={premium}
                  className="-top-2 -left-2 sm:-top-1.5 sm:-left-2.5 md:-top-1 md:-left-3 lg:top-3 lg:-left-2"
                />
                <img
            ...
            <div
              className={`z-40 absolute lg:relative lg:z-10 flex-col justify-center items-center lg:flex-row top-0 left-0 right-0 bg-customPalette-black lg:bg-transparent px-4 py-6 transition-all duration-300 ease-in-out h-screen lg:h-auto ${
